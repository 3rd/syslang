...
`)
	...
	assert.Equal(s.T(), 0*time.Second, tasks[0].GetTotalSessionDuration())
	...
	assert.InDelta(s.T(), time.Since(tasks[2].Sessions[0].Start).Seconds(), tasks[2].GetTotalSessionDuration().Seconds(), 1)
	...
	assert.Equal(s.T(), 25*time.Hour, tasks[3].GetTotalSessionDuration())
	...
	assert.Equal(s.T(), 0*time.Second, tasks[4].GetTotalSessionDuration())
